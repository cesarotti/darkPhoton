...
double ENERGY = 500.; //energy in MeV (if known)
double XPOS = 13.;
double YPOS = 0.;
double CRYStoMM = 50.;
...
vector<pair<int, double> > energySort2(vector<pair<int, double> > shower)
{
...
void resolutionNoise(double noiseLevel, bool clusterTest, int crysNum) 
{ 
  ...
  TH1D* energyReso = new TH1D("energyReso", "Energy_resolution",  1000, -500, 500);
  ...
  for (int i = 0; i < nEvents; i++)
    {
      ...
      trueEnergy = 0;
      double mostEnergetic = 0.0;
      for(int w = 0; w < 1225; w++)
	{
	  double noise = 0.0;
	  if (clusterTest) {noise = randomGen->Gaus(0, noiseLevel);}
	...
	{
      sortedlist = energySort2(geant);
      detector.push_back(sortedlist);
	}
      ...
      for (vv_iter=detector.begin(); vv_iter!=detector.end(); vv_iter++)
	{
	  ...
	  // if (localMax.size() ==1) We want to skip all other things 
	  if (true)
      ...
      for (vv_iter=detector2.begin(); vv_iter!=detector2.end(); ++vv_iter)
	{
	  ...
	  int num = crystalNumOptimized(&shower);
	  if (shower.size()>num)
	...
	{
	  vector<pair<int, double> > shower = sortedlist;
	  int num = crysNum;
	  if (shower.size()>num)
	    {shower.erase(shower.begin()+num, shower.end());}
	  ordered.push_back(shower);
