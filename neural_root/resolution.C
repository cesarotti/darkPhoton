...
void resolution() {
  ...
  TFile* file = new TFile("Signal_t0.root");

  TTree* crystal_entries = (TTree *)file->Get("Signal");
  TTree* perfect_entries = (TTree *)file->Get("Check");

  int crystalEvents = tree->GetEntries();
  int perfectEvents = tree->GetEntries();
  ...
  for (int k=0; k<1225; k++){
    ...
    crystal_entries->SetBranchAddress(charstr, &addresses[k]);
  }
